...
boys_dropout = [(total_boys[i]-total_boys[i+1])/total_boys[i+1] for i in range(len(total_boys)-1)]
girls_dropout = [(total_girls[i]-total_girls[i+1])/total_girls[i+1] for i in range(len(total_girls)-1)]
...
#Comparison across General vs ST vs SC vs OBC
...
plt.xlabel("Class")
plt.ylabel("Number of Students Enrolled")
plt.legend(["Scheduled Tribe", "Scheduled Caste", "Other Backward Classes", "General"])
plt.show()
